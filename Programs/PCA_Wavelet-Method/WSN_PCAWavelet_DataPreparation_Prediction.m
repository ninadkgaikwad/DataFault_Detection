function [  WSN_PCAWavelet_Predict_Normalized_DataCell,WSN_PCAWavelet_TargetOriginal_DataCell ] = WSN_PCAWavelet_DataPreparation_Prediction( SensorDataMatrixLabelled_Cell, WindowSize, ContinuousWindow_YesNo, Wavelet_Level, Wavelet_Filter, FeatureEnergySignal_YesNo, Normalization_YesNo, Normalization_TrainPredict, Training_Averages, Training_SD )
...
WSN_PCAWavelet_TargetPredict_DataCell1=Windowed_DataMatrix_Cell;

% Getting Size of WSN_PCA_Predict_DataCell

[R7,C7]=size(WSN_PCAWavelet_TargetPredict_DataCell1);

% Getting Size of Contents of WSN_PCA_Predict_DataCell

Contents1=WSN_PCAWavelet_TargetPredict_DataCell1{1,1};

[R8,C8]=size(Contents1);

% Intializing WSN_PCA_Predict_DataCell

WSN_PCAWavelet_TargetOriginal_DataCell=cell(1,C8);

% Creating WSN_PCA_Predict_DataCell

for i=1:C8 % For Each Column in Contents Matrix
    
    % Initializing WSN_PCA_Predict_Matrix

    WSN_PCAWavelet_Predict_Matrix1=zeros(R8,C7);
    
   for j=1:C7 % For Each Sensor_Data_Matrix in WSN_PCA_TargetPredict_DataCell 
       
       Data_Matrix=WSN_PCAWavelet_TargetPredict_DataCell1{1,j}; % Gettig the Sensor_Data_Matrix
       
       Data_Column_Vector=Data_Matrix(:,i); % Getting the Appropriate Column from the Appropriate Matrix
      
       WSN_PCAWavelet_Predict_Matrix1(:,j)=Data_Column_Vector; % Putting the Column Vector in Approptiate column location in the WSN_PCA_Predict_Matrix
       
   end
    
   % Putting the WSN_PCA_Predict_Matrix in appropriate location in the WSN_PCA_Predict_DataCell
   
   WSN_PCAWavelet_TargetOriginal_DataCell(1,i)=WSN_PCAWavelet_Predict_Matrix1;
...
if (FeatureEnergySignal_YesNo==1)
    ...
    for j=1:C3 % For each Wavelet_AverageCoefficients_Matrix within the Cell
       ...
       for i=1:C4 % For each Coulumn inside Wavelet_AverageCoefficients_Matrix
           ...
           EnergyFeature_Vector=AverageCoefficients_Vector.*AverageCoefficients_Vector;
...
WSN_PCAWavelet_TargetPredict_DataCell=ANN_Wavelet_Features_Matrix_Cell;

% Getting Size of WSN_PCA_Predict_DataCell

[R5,C5]=size(WSN_PCAWavelet_TargetPredict_DataCell);

% Getting Size of Contents of WSN_PCA_Predict_DataCell

Contents=WSN_PCAWavelet_TargetPredict_DataCell{1,1};

[R6,C6]=size(Contents);

% Intializing WSN_PCA_Predict_DataCell

WSN_PCAWavelet_Predict_DataCell=cell(1,C6);
...
for i=1:C6 % For Each Column in Contents Matrix
    ...
    WSN_PCAWavelet_Predict_Matrix=zeros(R6,C5);
    
   for j=1:C5 % For Each Sensor_Data_Matrix in WSN_PCA_TargetPredict_DataCell 
       
       Data_Matrix=WSN_PCAWavelet_TargetPredict_DataCell{1,j}; % Gettig the Sensor_Data_Matrix
       
       Data_Column_Vector=Data_Matrix(:,i); % Getting the Appropriate Column from the Appropriate Matrix
      
       WSN_PCAWavelet_Predict_Matrix(:,j)=Data_Column_Vector; % Putting the Column Vector in Approptiate column location in the WSN_PCA_Predict_Matrix
       
   end
    
   % Putting the WSN_PCA_Predict_Matrix in appropriate location in the WSN_PCA_Predict_DataCell
   
   WSN_PCAWavelet_Predict_DataCell(1,i)=WSN_PCAWavelet_Predict_Matrix;
   
end

% Initializing WSN_PCAWavelet_Predict_Normalized_DataCell

WSN_PCAWavelet_Predict_Normalized_DataCell=cell(1,C6);
...
if (Normalization_YesNo==1) % Perform Normalization
    ...
    if (Normalization_TrainPredict==1) % Normalization using Training Averages and Training Standard Deviations
        ...
        for i=1:C6 % For each Matrix in WSN_PCAWavelet_Predict_DataCell
            ...
            AppropriateMatrix=WSN_PCAWavelet_Predict_Normalized_DataCell{1,i};
           ...
           WSN_PCAWavelet_Predict_Normalized_DataCell(1,i)={Data_Matrix};
...
elseif (Normalization_YesNo==0) % Do not Perform Normalization
    ...
    WSN_PCAWavelet_Predict_Normalized_DataCell=WSN_PCAWavelet_Predict_DataCell;

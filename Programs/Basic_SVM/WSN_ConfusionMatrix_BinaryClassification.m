function [ Confusion_Matrix_Values, Confusion_Matrix_Percent, Rate_Matrix, PredictiveValue_Matrix, TotalAccuracy, TotalInAccuracy ] = WSN_ConfusionMatrix_BinaryClassification( Outputs, Targets, Target_Vector )
...
True_Target=Target_Vector(1,2);

False_Target=Target_Vector(1,2);
...
for i=1:R
   ...
   if ((Outputs(i,1)==True_Target)&&(Targets(i,1)==True_Target)) % Checking for True Positive
       
       TP=TP+1;
       
   elseif ((Outputs(i,1)==False_Target)&&(Targets(i,1)==False_Target)) % Checking for True Negative
       
       TN=TN+1;
       
   elseif ((Outputs(i,1)==True_Target)&&(Targets(i,1)==False_Target)) % Checking for False Positive
       
       FP=FP+1;
       
   elseif ((Outputs(i,1)==False_Target)&&(Targets(i,1)==True_Target))% Checking for False Negative
